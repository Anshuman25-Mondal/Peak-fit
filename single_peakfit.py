...
os.chdir('D:/NMO_data/low_temperature/data_together') #path
myfiles = [f for f in os.listdir() if f.endswith(".xye")]
...
starting_file=0. 
end_file=1 #can be used for handling multiple files together, use '1' for 1 file. 

wavelength=0.35433272
...
def pv(x,a1,a2,a3,a4,a5,a6):
    y = a1 + a3*(1 - a2)*(np.sqrt(4*np.log(2))/(np.sqrt(np.pi)*a5))*np.exp(-(4*np.log(2)/a5**2)*(x-a4)**2)+ a3*a2*(2/np.pi)*(a5/(4*(x-a4)**2 + a5**2))+a6*x

    return y

plt.figure(2)
plt.plot(x_exp,y_exp)
pts2=np.array(plt.ginput(1))
plt.close(2)

a0=[bk,0.5,pts2[0,1],pts2[0,0],0.05,bk]
...
results_array=np.zeros([len(myfiles),8])
...
for p in range(starting_file+1,end_file):
    ...
    x_exp = twotheta[int(results_array[p-1,6]):int(results_array[p-1,7])]
    y_exp = intensity[int(results_array[p-1,6]):int(results_array[p-1,7])]
    
    bk=(np.mean(intensity[int(results_array[p-1,6]):int(results_array[p-1,6]+3)])+np.mean(intensity[int(results_array[p-1,7]-3):int(results_array[p-1,7])]))/2
    a0 = [bk, results_array[p-1,1], results_array[p-1,2],results_array[p-1,3],results_array[p-1,4],results_array[p-1,5]]
    ...
    pts_0_moving = results_array[p-1,6]+idx_high-idx_low
    pts_1_moving = results_array[p-1,7]+idx_high-idx_low
